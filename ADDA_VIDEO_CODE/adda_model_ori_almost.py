...
def parse_args():
    ...
    parser.add_argument('--target_weights', dest='target_pretrained_model',
                        ...
                        default='/home/charan/target_rcnn/VGGnet_fast_rcnn_iter_70000.ckpt', type=str)
    parser.add_argument('--source_weights', dest='source_pretrained_model',
                        help='initialize with pretrained model weights',
                        default='/home/charan/source_rcnn/VGGnet_fast_rcnn_iter_70000.ckpt', type=str)
...
class SolverWrapper(object):
	...
	adversary_logits = custom_adversarial_discriminator(adversary_ft, [500,500], leaky=True)
	...
	discriminator_step = optimizer.minimize(adversary_loss, var_list=list(adversary_vars.values()))
       	target_step = optimizer_t.minimize(mapping_loss, var_list=list(target_vars.values()))
	...
	f.close()
		...
		self.source_network.data: source_blobs['data'], self.source_network.im_info: source_blobs['im_info'], self.source_network.keep_prob: 1.0, self.source_network.gt_boxes: source_blobs['gt_boxes']}
            ...
            target_loss_val, disc_loss_val, _, _ = sess.run(
            [mapping_loss, adversary_loss, target_step, discriminator_step], feed_dict = feed_dict)
